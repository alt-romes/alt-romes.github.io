...
description:
    ...
    applications with functional programming, in Haskell, using the
    <strong>Model-View-Controller</strong> pattern in its functional flavour.
    The <strong>MVC</strong> pattern is common accross all languages and goes
    by many names. For example, it's how smartphone applications are structured
    in Android Studio.
